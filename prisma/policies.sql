-- =====================================================================
-- FriendIntro - Row Level Security policies (production)
-- Complete policy set aligned with prisma/schema.prisma (202612 hardening)
--
-- Run manually after `prisma migrate deploy`:
--   npm run db:rls
...
-- Service role or assigned RBAC admin role (defense-in-depth for Supabase clients).
...
as $$
  select exists (
    ...
    where ur.user_id = auth.uid()
      and r.name in ('SuperAdmin', 'Admin', 'Moderator', 'Support', 'Analyst')
  );
$$;
...
-- ---------------------------------------------------------------------
-- RBAC (read via app; writes server/service only)
-- ---------------------------------------------------------------------
...
drop policy if exists permissions_mutate_admin on public.permissions;
create policy permissions_mutate_admin
...
-- ---------------------------------------------------------------------
-- ADMIN AUDIT LOGS (append-only via service; admins read)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- SECURITY EVENTS (admins read; system writes)
-- ---------------------------------------------------------------------
